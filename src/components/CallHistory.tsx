...
export const CallHistory: React.FC<CallHistoryProps> = ({ onCallNumber }) => {
      ...
      <div className="bg-white/90 backdrop-blur-lg rounded-2xl xs:rounded-3xl shadow-strong border border-white/50 p-4 xs:p-5 w-full animate-fade-in-up mobile-ui-only">
        <div className="text-center py-8 xs:py-10">
          <div className="inline-flex items-center justify-center w-16 h-16 xs:w-20 xs:h-20 rounded-full bg-gradient-to-br from-secondary-100 to-secondary-200 mb-4 shadow-soft">
            <Clock className="w-8 h-8 xs:w-10 xs:h-10 text-secondary-500" />
          </div>
          <h3 className="text-lg xs:text-xl font-bold text-secondary-900 mb-2 bg-gradient-to-r from-secondary-700 to-secondary-900 bg-clip-text text-transparent">No Call History</h3>
  ...
  return (
    <div className="bg-white/90 backdrop-blur-lg rounded-2xl xs:rounded-3xl shadow-strong border border-white/50 p-4 xs:p-5 w-full animate-fade-in-up mobile-ui-only">
      <div className="flex items-center gap-2 xs:gap-2.5 mb-4 xs:mb-5">
        ...
        <h3 className="text-lg xs:text-xl font-bold bg-gradient-to-r from-secondary-800 to-secondary-900 bg-clip-text text-transparent">Recent Calls</h3>
      </div>
      
      <div className="space-y-2 xs:space-y-2.5 max-h-[60vh] overflow-y-auto">
        {callHistory.map((call) => (
          <div
            key={call.id}
            className="flex items-center gap-2.5 xs:gap-3 p-3 xs:p-3.5 rounded-xl xs:rounded-2xl bg-gradient-to-r from-white to-secondary-50/50 hover:from-primary-50 hover:to-primary-100/50 active:from-primary-100 active:to-primary-200/50 border border-secondary-200/50 hover:border-primary-300/50 shadow-soft hover:shadow-medium transition-all duration-300 group touch-manipulation smooth-hover"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] xs:text-xs text-secondary-500 flex-shrink-0">
                  {formatTimestamp(call.timestamp)}
                </span>
              </div>
              <div className="flex items-center justify-between mt-0.5 xs:mt-1 gap-2">
                <p className="text-[10px] xs:text-xs text-secondary-500">
            ...
            <button
              ...
              className="flex-shrink-0 w-9 h-9 xs:w-10 xs:h-10 rounded-full bg-gradient-to-br from-success-500 to-success-600 hover:from-success-600 hover:to-success-700 active:from-success-700 active:to-success-800 text-white flex items-center justify-center opacity-100 shadow-medium hover:shadow-strong hover:scale-110 active:scale-95 transition-all duration-300 touch-manipulation smooth-hover"
            >
