...
export const DialerTabs: React.FC = () => {
  ...
  return (
    <div className="w-full mobile-ui-only">
      {/* Tab Navigation - Mobile UI for all screens */}
      <div className="flex bg-gradient-to-r from-secondary-100 to-secondary-200 rounded-xl xs:rounded-2xl p-1 xs:p-1.5 mb-3 xs:mb-4 shadow-soft border border-secondary-300/30">
        {tabs.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setActiveTab(id)}
            className={`flex-1 flex items-center justify-center gap-1.5 xs:gap-2 py-2.5 xs:py-3 px-1 xs:px-2 rounded-lg xs:rounded-xl text-[10px] xs:text-xs font-semibold transition-all duration-300 touch-manipulation min-h-[48px] smooth-hover ${
              activeTab === id
          ...
          >
            <Icon className={`w-4 h-4 xs:w-4.5 xs:h-4.5 transition-transform ${activeTab === id ? 'scale-110' : ''}`} />
